...
Registry operation-5 validation, accepted-state integration, and independent remote read-back are implemented. `RegistryService.confirm_identity_owner_key_rotation(owner_name_hex=..., expected_envelope_cbor=...)` returns an `IdentityRecordResult` only when a fresh cache-empty DHT client reads bytes exactly equal to the expected finalized envelope and the Registry replays its complete accepted predecessor chain from a signed anchor. The fresh client has no durable store and does not use the writer's DHT value store or write result. It is bootstrapped to peers previously supplied through `Libp2pKadDHT.bootstrap(...)`; without configured peers or a successful exact remote response, the method returns `None`. This single-peer read proves only that a remote DHT peer returned the exact value at read time; it does not prove quorum replication, global currentness, or network-wide durability. The method returns validated public record data, not a wallet `RotationConfirmation`. Wallet private-key handling, consent, dispatch latching, ambiguous-outcome resolution, publication finalization, and successor-key promotion remain unimplemented; wallet issue #18 remains open, and Registry acceptance/read-back alone is not wallet confirmation.
